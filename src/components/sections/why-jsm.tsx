...
import { ClipboardCheck, Cog, Eye, Shield, Lightbulb, Award, Clock, ArrowRight } from 'lucide-react';
...
export function WhyJSM() {
  ...
  return (
    <section className="py-24 bg-white border-b border-zinc-200/80 relative z-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center mb-24">
          ...
          >
            {processSteps.map((step, idx) => (
              <motion.div 
                key={idx} 
                variants={fadeInUp}
                className="flex gap-4 p-6 bg-zinc-50 rounded-2xl border border-zinc-200/60 hover:border-[#C5A880]/30 transition-all duration-300 group"
              >
                <div className="w-12 h-12 rounded-xl bg-white border border-zinc-200 text-[#C5A880] flex items-center justify-center flex-shrink-0 group-hover:bg-[#C5A880] group-hover:text-white transition-colors duration-300">
          ...
          >
            ...
            <div className="absolute top-6 left-6 md:top-8 md:left-8 bg-black/80 backdrop-blur-md text-white px-4 py-2 rounded-full text-xs font-semibold uppercase tracking-widest">
        ...
        {/* Values Grid */}
        <div>
          <h3 className="text-xl font-bold text-black/40 uppercase tracking-widest text-center mb-10">Our Operational Values</h3>
          <motion.div 
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
          ...
          >
            {values.map((val, idx) => (
              <motion.div 
                key={idx}
                variants={fadeInUp}
                className="p-6 rounded-2xl bg-zinc-50 border border-zinc-200/60 flex flex-col items-center text-center hover:shadow-md transition-shadow duration-300"
              >
                <div className="w-12 h-12 rounded-xl bg-white border border-zinc-200 flex items-center justify-center text-[#C5A880] mb-4">
